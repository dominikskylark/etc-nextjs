...
export default () => {
  ...
  days.length = Number(context.states.eventOptions.number_of_days);
    ...
    <>
      <div className="row">
        <ul className="nav nav-pills nav-fill">
          {context.states.eventOptions.has_conference_room_one ? (
            <li className="nav-item" onClick={() => setWhichRoom(1)}>
              <a className={"nav-link " + (whichRoom == 1 ? "active" : null)}>
                <h3>Room 1</h3>
                {context.states.eventOptions.room_one_name}
              </a>
            </li>
          ) : null}
          {context.states.eventOptions.has_conference_room_two ? (
            <li className="nav-item" onClick={() => setWhichRoom(2)}>
              <a className={"nav-link " + (whichRoom == 2 ? "active" : null)}>
                <h3>Room 2</h3>
                {context.states.eventOptions.room_two_name}
              </a>
            </li>
          ) : null}
          {context.states.eventOptions.has_conference_room_three ? (
            <li className="nav-item" onClick={() => setWhichRoom(3)}>
              <a className={"nav-link " + (whichRoom == 3 ? "active" : null)}>
                <h3>Room 3</h3>
                {context.states.eventOptions.room_three_name}
              </a>
            </li>
          ) : null}
          {context.states.eventOptions.has_conference_room_four ? (
            <li className="nav-item" onClick={() => setWhichRoom(4)}>
              <a className={"nav-link " + (whichRoom == 4 ? "active" : null)}>
                <h3>Room 4</h3>
                {context.states.eventOptions.room_four_name}
              </a>
            </li>
          ) : null}
        </ul>
            ...
            ? roomDetails.acf[`sessions_day_${whichDay}`].map((session) => {
                return (
                  <AgendaListItem
                    ...
                    day={whichDay}
                    dayDate={dateOfDay}
                    options={context.states.eventOptions}
                  />
                );
              })
            : null}
